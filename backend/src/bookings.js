...
function matchType(request, messageContent) {
  ...
  return `What day of the week do you want to see the ${request.session.type}?`;
}
...
function matchDay(request, messageContent) {
  ...
  return `What time on ${capitalize(day[0])}:\n
          10am, 11am, 1pm, 4pm`;
}
...
function matchTime(request, messageContent) {
  ...
  return `Your appointment to see the ${type} on ${capitalize(weekday)} at ${
    match[0]
  } was made, please let us know if you need to change the time, otherwise see you on ${capitalize(
    weekday
  )}.`;
}
...
function confirmBooking(request) {
  ...
  return `Your appointment is booked to see the ${type} on ${capitalize(
    weekday
  )} at ${time}. If you want to change it, please contact us at 555-5555`;
}
